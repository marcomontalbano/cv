...
describe('ItemsWithCompany', () => {
  it('should render a list of items', () => {
    const { queryByText, getAllByText } = render(
      <ItemsWithCompany items={experiences.asList} />,
    )
    expect(getAllByText('John Doe SpA, Binasco, Milan (Italy)').length).toEqual(
      2,
    )
  ...
  it('should not render the item if not visible', () => {
    const { queryByText, getAllByText } = render(
      <ItemsWithCompany items={experiences.withoutVisibleFalse} />,
    )
    expect(getAllByText('John Doe SpA, Binasco, Milan (Italy)').length).toEqual(
      1,
    )
  ...
  it('should not render the endDate if equal to startDate render a list of items', () => {
    const { queryByText, getAllByText } = render(
      <ItemsWithCompany items={experiences.withSameMonth} />,
    )
    expect(getAllByText('John Doe SpA, Binasco, Milan (Italy)').length).toEqual(
      1,
    )
  ...
  it('should not render the address if not present', () => {
    const { getAllByText } = render(
      <ItemsWithCompany items={experiences.withoutAddress} />,
    )
  ...
  it('should render "present day" if the endDate is not present', () => {
    const { queryByText } = render(
      <ItemsWithCompany items={experiences.withoutEndDate} />,
    )
    expect(queryByText('June 2016')).not.toBeNull()
  ...
  it('should render the sidebar title', () => {
    const { queryByText } = render(
      <ItemsWithCompany items={[]} title="This is the title!" />,
    )
    expect(
